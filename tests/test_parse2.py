...
class Test_P2_MemberExpression_PrimaryExpression:
    ...
    @strict_params
    def test_IsDestructuring(self, mocker, context, kind, expected, strict):
        ...
        assert me.IsDestructuring() == expected
...
class Test_P2_MemberExpression_MemberExpression_LBRACKET_Expression_RBRACKET:
    ...
    #   1. Return false.
    @strict_params
    def test_IsFunctionDefinition(self, mocker, context, strict):
        me = e.P2_MemberExpression_MemberExpression_LBRACKET_Expression_RBRACKET(
            context, strict, [mocker.Mock(), mocker.Mock(), mocker.Mock(), mocker.Mock()]
        )
        assert me.IsFunctionDefinition() is False

    # 12.3.1.4 Static Semantics: IsDestructuring
    # MemberExpression : MemberExpression [ Expression ]
    #   1. Return false.
    @strict_params
    def test_IsDestructuring(self, mocker, context, strict):
        me = e.P2_MemberExpression_MemberExpression_LBRACKET_Expression_RBRACKET(
            context, strict, [mocker.Mock(), mocker.Mock(), mocker.Mock(), mocker.Mock()]
        )
        assert me.IsDestructuring() is False

    # 12.3.1.5 Static Semantics: IsIdentifierRef
    # MemberExpression : MemberExpression [ Expression ]
    #   1. Return false.
    @strict_params
    def test_IsIdentifierRef(self, mocker, context, strict):
        me = e.P2_MemberExpression_MemberExpression_LBRACKET_Expression_RBRACKET(
            context, strict, [mocker.Mock(), mocker.Mock(), mocker.Mock(), mocker.Mock()]
        )
        assert me.IsIdentifierRef() is False

    # 12.3.1.6 Static Semantics: AssignmentTargetType
    # MemberExpression : MemberExpression [ Expression ]
    #   1. Return simple.
    @strict_params
    def test_AssignmentTargetType(self, mocker, context, strict):
        me = e.P2_MemberExpression_MemberExpression_LBRACKET_Expression_RBRACKET(
            context, strict, [mocker.Mock(), mocker.Mock(), mocker.Mock(), mocker.Mock()]
        )
        assert me.AssignmentTargetType == e.SIMPLE
...
class Test_parse_MemberExpression:
    # Syntax
    ...
    def setup_prod_mocks(cls, mocker, token_stream, lex_pos):
        return prod_mocks(
            ...
            set(
                p
                for p in chain.from_iterable(prod for prod, ecls in cls.productions)
    ...
    def test_syntax_errors(self, mocker, context, strict_flag, yield_flag, await_flag, token_stream, lex_pos):
        ...
        me = e.parse_MemberExpression(context, lexer, lex_pos, strict_flag, yield_flag, await_flag)

        if len(token_stream) > 1 and token_stream[0] == "PrimaryExpression":
            expected_type = self.ME_PE
        else:
            expected_type = type(None)
        assert isinstance(me, expected_type)
    ...
    def test_ordinary(
        self, mocker, context, strict_flag, yield_flag, await_flag, token_stream, expected_class, guard, lex_pos
    ):
        lexer = lexer_mock(mocker, token_stream, lex_pos)
        prod_mocks = self.setup_prod_mocks(mocker, token_stream, lex_pos)
        extra_args = {
            "PrimaryExpression": (yield_flag, await_flag),
            "Expression": (True, yield_flag, await_flag),
            "TemplateLiteral": (yield_flag, await_flag, True),
            "SuperProperty": (yield_flag, await_flag),
            "MetaProperty": (),
            "Arguments": (yield_flag, await_flag),
        }

        me = e.parse_MemberExpression(context, lexer, lex_pos, strict_flag, yield_flag, await_flag)

        assert isinstance(me, expected_class)
        for name, mock in prod_mocks.items():
            if name in token_stream:
                mock.assert_called_with(context, lexer, mocker.ANY, strict_flag, *extra_args[name])
        assert me.strict == strict_flag
        assert len(me.children) == len(token_stream)
        for idx, token in enumerate(token_stream):
            try:
                value = me.children[idx].value
            except AttributeError:
                value = me.children[idx].children[0].value
